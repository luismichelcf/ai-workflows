...
describe('a verdict of an older version names both versions (CN-03)', () => {
  ...
  it('with several older verdicts it names the newest one', async () => {
    const H0 = '9999999ddddddddddddddddddddddddddddddddd';
    const decision = await decide([builder, verdictOf(H1, '2026-09-26T01:09:00Z'), verdictOf(H0, '2026-09-26T01:02:00Z')], true);
    expect(decision.ok).toBe(false);
    if (decision.ok) return;
    expect(decision.reason).toContain('1111111');
